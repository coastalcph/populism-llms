...
from transformers.models.roberta.modeling_roberta import RobertaForSequenceClassification, RobertaSelfAttention, \
    RobertaClassificationHead

# from transformers.models.roberta.modeling_roberta import ROBERTA_START_DOCSTRING, ROBERTA_INPUTS_DOCSTRING, \
#     _CONFIG_FOR_DOC

from transformers.utils.args_doc import auto_docstring
# from transformers.models.roberta.modeling_roberta import _CONFIG_FOR_DOC
from transformers.models.roberta.configuration_roberta import RobertaConfig

from xai.xai_utils.layer_utils import LayerNormXAI, ActivationXAI, GELUActivationXAI
...
class RobertaForSequenceClassificationXAI(RobertaForSequenceClassification):
    ...
    def override_default_layers(self, config):
        ...
        for i in range(config.num_hidden_layers):
            ...
            self.roberta.encoder.layer[i].attention.output.LayerNorm = LayerNormXAI((config.hidden_size,),
                                                                                    eps=config.layer_norm_eps,
                                                                                    args=largs)

            self.roberta.encoder.layer[i].intermediate.intermediate_act_fn = GELUActivationXAI()

            self.roberta.encoder.layer[i].output.LayerNorm = LayerNormXAI((config.hidden_size,),
                                                                          eps=config.layer_norm_eps, args=largs)
